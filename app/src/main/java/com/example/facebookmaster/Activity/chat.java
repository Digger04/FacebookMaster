...
public class chat extends AppCompatActivity {
    ...
    public void chat_admin(View view) {
        Intent intent = new Intent(getApplicationContext(), chat_admin.class);
    ...
    public void send_tn(View view) {
        if (edit_nhaptn.getText().toString().length() > 0) {
            ...
            FirebaseDatabase.getInstance().getReference().child("chat").child("all").child(System.currentTimeMillis() + "").setValue(model_tinnhan)
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable final Intent data) {
        ...
        if (requestCode == 123 && resultCode == RESULT_OK && data != null) {
            FirebaseStorage.getInstance().getReference().child("chat").child("all").child(System.currentTimeMillis() + "")
    ...
    private void get_data_tinnhan() {
        ...
        FirebaseDatabase.getInstance().getReference().child("chat").child("all").addChildEventListener(new ChildEventListener() {
            @Override
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                if (dataSnapshot.exists()) {
